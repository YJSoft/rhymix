...
class hCaptcha
{
	protected static $verify_url = 'https://api.hcaptcha.com/siteverify';
	...
	public static function check($response = null)
	{
		if (!$response)
		{
			$response = Context::get('h-captcha-response');
		}
		if (!$response)
		{
	...
	public function addScripts()
	{
		if (!self::$scripts_added)
		{
			...
			Context::loadFile(array('./modules/spamfilter/tpl/js/hcaptcha.js', 'body'));
			Context::addHtmlFooter('<script src="https://js.hcaptcha.com/1/api.js?render=explicit&amp;onload=hCaptchaCallback" async defer></script>');
			$html = '<div id="hcaptcha-config" data-sitekey="%s" data-theme="%s" data-size="%s" data-targets="%s"></div>';
	...
	{
		return sprintf('<div id="hcaptcha-instance-%d" class="h-captcha"></div>', self::$instances_inserted++);
	}
}
